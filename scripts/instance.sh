...
if [ ${node_id} -eq 1 ]; then
    ...
    /opt/redislabs/bin/rladmin cluster create name ${cluster_dns} username ${RS_admin} password "${RS_password}" external_addr $node_external_addr
else
    echo "joining cluster " >> /home/ubuntu/install.log
    /opt/redislabs/bin/rladmin cluster join username ${RS_admin} password "${RS_password}" nodes ${node_1_ip} external_addr $node_external_addr
fi
