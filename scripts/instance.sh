...
if [ ${node_id} -eq 1 ]; then
    ...
    for i in {1..10}
    do
	    /opt/redislabs/bin/rladmin cluster join username ${RS_admin} password '${RS_password}' nodes ${node_1_ip} external_addr $node_external_addr flash_enabled replace_node ${node_id} rack_id '${zone}' 2>&1 >> /home/ubuntu/install.log
    	if [ $? -eq 0 ]; then
	        break
    	else
